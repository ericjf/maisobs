...
static std::string dpapi_encrypt_b64(const std::string &plain)
{
	...
	DATA_BLOB out{};
	if (!CryptProtectData(&in, L"obs-stream-sync", nullptr, nullptr, nullptr, 0, &out)) {
		obs_log(LOG_WARNING, "[stream-sync] DPAPI encrypt failed 0x%lx", (unsigned long)GetLastError());
		return plain;
	}
	DWORD n = 0;
...
static std::string dpapi_decrypt_b64(const std::string &b64)
{
	...
	if (!CryptProtectData(&in, nullptr, nullptr, nullptr, nullptr, 0, &out)) {
		obs_log(LOG_WARNING, "[stream-sync] DPAPI decrypt failed 0x%lx", (unsigned long)GetLastError());
		return b64;
	}
...
void save(const std::string &provider, const std::string &access_token, const std::string &refresh_token)
{
	...
	if (!f) {
		obs_log(LOG_WARNING, "[stream-sync] cannot write token file: %s", path.c_str());
		return;
	}
	f << enc_access << "\n" << enc_refresh << "\n";
}
