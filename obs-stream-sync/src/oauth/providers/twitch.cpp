...
static OAuthFlow::Config make_config(const QString &client_id, const QString &client_secret)
{
	...
	c.scopes = {"channel:manage:broadcast", "user:read:email"};
	return c;
}
	  ...
	  http_(new HttpClient(this))
{
	...
	char *path = obs_module_config_path("twitch_app.json");
	if (path && os_file_exists(path)) {
		obs_data_t *d = obs_data_create_from_json_file(path);
		if (d) {
			client_id_ = obs_data_get_string(d, "client_id");
			client_secret_ = obs_data_get_string(d, "client_secret");
			obs_data_release(d);
		}
	}
	bfree(path);
	...
	if (client_id_.isEmpty())
		obs_log(LOG_WARNING,
			"[stream-sync] Twitch client_id not configured. "
			"Create %s/twitch_app.json with {\"client_id\":\"...\",\"client_secret\":\"...\"}",
...
void TwitchProvider::connect(std::function<void(bool, const QString &)> cb)
{
	...
	flow_->start(make_config(client_id_, client_secret_), [this, cb](bool ok, const QString &at,
								       const QString &rt, const QString &err) {
		if (!ok) {
			cb(false, err);
			return;
		}
...
void TwitchProvider::fetch_user_info(std::function<void(bool)> cb)
{
	...
	http_->get(QUrl(QString(API_BASE) + "/users"), h,
		   [this, cb](int status, const QByteArray &body, const QString &) {
			   ...
			   obs_log(LOG_INFO, "[stream-sync] Twitch connected: %s (id=%s)",
				    ...
				    std::function<void(bool, const QString &)> cb)
{
	...
	auto do_update = [this, title, cb](const QString &game_id) {
		...
		http_->patch(url, h, body, [this, cb, title, body](int status, const QByteArray &resp,
							      const QString &) {
			if (status == 204 || status == 200) {
				obs_log(LOG_INFO, "[stream-sync] Twitch channel updated: %s",
